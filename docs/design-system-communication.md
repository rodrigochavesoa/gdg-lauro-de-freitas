...
- **`/eventos` e `/newsletter`** reutilizam o shell da Home (hero pontilhado + curva DS-07 **sem** avatar): busca/filtros e cards no índice de eventos; placeholder de inscrição na newsletter. Contratos: [`ux-shell-eventos.md`](ux-shell-eventos.md) · [`ux-shell-newsletter.md`](ux-shell-newsletter.md). Merge #65–#74 (`main` @ `bf8b204`).
