...
Migrations: pasta [`supabase/migrations/`](supabase/migrations/). Homologação aplica a cadeia completa (inclui seed). Produção aplica só o manifesto [`supabase/migrations/prod.manifest.json`](supabase/migrations/prod.manifest.json) (`pnpm migrations:prod`) — **sem** seed fictício e **sem** arquivos marcados «Produção: não aplicar».
