...
A raiz de `supabase/migrations/` é o path que `supabase db push` aplica. `supabase/migrations/prod.manifest.json` é a fonte de verdade da lista autorizada para produção. SQL homolog-only fica em `supabase/migrations/homolog/` (o CLI não lê subpastas).

`pnpm migrations:prod` valida e lista migrations; ele não aplica automaticamente mudanças em produção. `pnpm migrations:homolog` lista a cadeia completa de homologação e também não aplica SQL. Uma migration nova deve ser classificada explicitamente. Arquivos homolog-only ou marcados “Produção: não aplicar” não entram no manifesto.
